...
interface Props {
  scenarios: Record<string, { emoji: string; name: string; leads: unknown[] }>;
            ...
            >
              ...
              <div>
                ...
                <span className="block text-xs text-[#7a8ba5]">
                  {key === "inmobiliaria" && "Visitas y cotizaciones"}
                  {key === "automotriz" && "Test drive y financiamiento"}
                  {key === "educacion" && "Matrículas y consultas"}
                  {key === "dental" && "Citas y presupuestos"}
                  {key === "muebles" && "Pedidos y delivery"}
                </span>
              </div>
            </button>
          );
